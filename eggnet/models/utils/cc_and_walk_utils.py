...
def add_track_labels(graph, all_trks):
    ...
    for method, trks in all_trks.items():
        for i, track in enumerate(trks):
            track_id = i + trkid_offset
            flat_trks.extend(track)
            flat_trkid.extend([track_id] * len(track))
            flat_method.extend([method] * len(track))
        trkid_offset += len(trks)

    num_nodes = int(graph.hit_id.shape[0])
    graph.hit_track_labels = torch.full(
        (num_nodes,),
        -1,
        dtype=torch.long,
        device=graph.hit_id.device,
    )
    graph.hit_reco_method = np.full(num_nodes, -1, dtype=object)

    if not flat_trks:
        return

    flat_hit_ids = torch.as_tensor(flat_trks, dtype=graph.hit_id.dtype, device=graph.hit_id.device)
    flat_track_ids = torch.as_tensor(flat_trkid, dtype=torch.long, device=graph.hit_id.device)
    sorted_hit_ids, sorted_indices = torch.sort(graph.hit_id)
    positions = torch.searchsorted(sorted_hit_ids, flat_hit_ids)
    valid = (positions < sorted_hit_ids.numel()) & (
        sorted_hit_ids[positions] == flat_hit_ids
    )
    if not bool(valid.any().item()):
        return

    node_indices = sorted_indices[positions[valid]]
    assigned = torch.zeros(num_nodes, dtype=torch.bool, device=graph.hit_id.device)
    valid_track_ids = flat_track_ids[valid]
    valid_positions = torch.nonzero(valid, as_tuple=False).flatten().tolist()
    valid_methods = [flat_method[i] for i in valid_positions]

    for node_index, track_id, method in zip(
        node_indices.tolist(),
        valid_track_ids.tolist(),
        valid_methods,
    ):
        if bool(assigned[node_index].item()):
            continue
        graph.hit_track_labels[node_index] = track_id
        graph.hit_reco_method[node_index] = method
